		...
		Thread = std::make_shared<CThread>(nullptr, NULL, [](LPVOID) -> DWORD
		{
			for (urmem::address_t baseAddress = NULL; baseAddress == NULL; std::this_thread::sleep_for(std::chrono::milliseconds(100)))
			{
				...
				if (baseAddress != NULL)
				{
					...
					urmem::address_t AddClientMessageAddress, FPSUnlockAddress, FPSLimitAddress, SleepHookAddress;
					...
					if (!scan.find("\x6A\x01\xFF\x15\x00\x00\x00\x00\xBA\x80\x1A\x56\x00", "xxxx????xxxxx", SleepHookAddress)) break;
...
std::shared_ptr<urmem::patch> FPSUnlock::FPSLimitPatch[3], FPSUnlock::gamePatch[2];
